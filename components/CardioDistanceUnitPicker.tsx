...
import {
  ...
  CARDIO_DISTANCE_UNIT_ABBREVIATIONS,
...
type Props = {
  ...
  onChange: (next: CardioDistanceUnit) => void;
  disabled?: boolean;
  borderColor: string;
  textColor: string;
};

export function CardioDistanceUnitPicker({ value, onChange, disabled = false, borderColor, textColor }: Props) {
  ...
  return (
    <>
      <Pressable
        ...
        accessibilityLabel={`Distance unit: ${CARDIO_DISTANCE_UNIT_LABELS[value]}. Tap to change.`}
        disabled={disabled}
        ...
        <Text style={[styles.buttonLabel, { color: textColor }]} numberOfLines={1}>
          {CARDIO_DISTANCE_UNIT_ABBREVIATIONS[value]}
        </Text>
      </Pressable>
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <Pressable style={styles.backdrop} onPress={() => setOpen(false)}>
          <Pressable
            ...
            <Text style={[styles.sheetTitle, { color: textColor }]}>Distance unit</Text>
              ...
              {CARDIO_DISTANCE_UNITS.map((unit) => {
                ...
                return (
                  <Pressable
                    ...
                    <Text style={[styles.optionLabel, { color: textColor }]}>{CARDIO_DISTANCE_UNIT_LABELS[unit]}</Text>
                    <Text style={[styles.optionAbbrev, { color: activeScheme === 'dark' ? '#a3a3a3' : '#737373' }]}>
                      {CARDIO_DISTANCE_UNIT_ABBREVIATIONS[unit]}
                    </Text>
                  </Pressable>
                );
              })}
